import {
  collection, doc, getDocsFromServer, runTransaction,
} from 'firebase/firestore';
import type {
  BackupRow, FinanceTable, FirestoreApplyResult, FirestoreCloudAdapter,
  FirestoreUpload,
} from '@f1nancer/domain';
import { firestore } from './firebaseClient';

function sameRow(left: BackupRow, right: BackupRow): boolean {
...
export class FirebaseCloudAdapter implements FirestoreCloudAdapter {
  ...
  async readAll(table: FinanceTable): Promise<BackupRow[]> {
    const snapshot = await getDocsFromServer(collection(firestore, 'users', this.userId, table));
    return snapshot.docs.map(item => ({ ...item.data(), id: item.id }) as BackupRow);
  }
  ...
  async apply(operations: FirestoreUpload[]): Promise<FirestoreApplyResult[]> {
    ...
    for (const operation of operations) {
      ...
      const result = await runTransaction(firestore, async transaction => {
        ...
        const remote = snapshot.exists() ? ({ ...snapshot.data(), id: snapshot.id } as BackupRow) : null;
